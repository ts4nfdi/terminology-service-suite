...
export default function RenderedReified({
  ...
  function RenderedValue({ value }: { value: any }): ReactElement {
    ...
    function addLinksToText(text: string) {
      ...
      for (const link of linksToSplice) {
        res.push(
          <span key={randomString()}>{text.substring(n, link.start)}</span>,
        );
        res.push(link.link);
        n = link.end;
      }
    ...
    if (Object.keys(linkedEntities.linkedEntities).length == 0) {
      if (typeof value == "string") {
        ...
        typeof value == "object" &&
        value["http://www.w3.org/1999/02/22-rdf-syntax-ns#type"] != undefined
      ) {
        // Is converted from Skosmos
        switch (value["http://www.w3.org/1999/02/22-rdf-syntax-ns#type"]) {
  ...
  function RenderedAxioms({
    ...
    const axiomsText = Object.keys(axiomsDict)
      .map((key) => {
        const label =
          parentEntity.getLinkedEntities().getLabelForIri(key) || key;
        if (label) {
